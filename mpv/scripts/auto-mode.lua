...
function on_image_mode_activate()
    ...
    mp.set_property("pause","yes")                             -- doesn't let images autoplay
	mp.set_property("hwdec","no")			       -- disable hardware decoding for images
    mp.set_property("background", "#000000")                   -- use dark grey background for images
...
function on_type_change(old_ext, new_ext)
    if new_ext == ".gif" then
		...
		mp.set_property("pause","no")-- loop GIF files
    end
...
image_mode_bindings = {
    { "UP",         function () mp.command("playlist-next") end,  "repeatable" }, -- move image up
    { "DOWN",       function () mp.command("playlist-prev") end,  "repeatable" }, -- move image down
...
image_file_extensions = { ".jpg", ".jpeg", ".png", ".bmp", "gif", ".webp", ".pdf" }
